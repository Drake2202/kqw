    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu">
        ...
        <li class="active treeview">
          ...
          <ul class="treeview-menu">
            <li class="active"><a href="reported.php"><i class="fa fa-circle-o"></i> Reported</a></li>
      ...
      <div class="box">
		...
		<div class="block">
			<div class="block full">
				...
				<h6 class="heading-hr"><i class="icon-file"></i> Search Reported</h6>
								</ul>
        	</div>
				<form action="" method="post">
					<div class="input-group">
						<input type="text" id="search-term" value="<?php echo addslashes($_POST['search-term']); ?>" name="search-term" class="form-control" placeholder="Post name">
		...
		<tr>
			...
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Reported by</b></td>
			<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align="center"><b>Suspect</b></td>
		...
		<?php
			if(!(isset($_POST['search-term']))){
				...
				$sql = "SELECT COUNT(*) AS total FROM `meh_users_reports`";
				...
				$sql = "SELECT * FROM `meh_users_reports` ORDER BY `id` ASC LIMIT ".$inicio.", ".$_BS['bypage'];
				$query = mysql_query($sql);

				while ($resultado = mysql_fetch_assoc($query)) {
					$id = $resultado['id'];
					$reported = $resultado['reported_by'];
					$suspect = $resultado['suspect'];
					$reason = $resultado['reason'];
					echo "
						<tr>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$id</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$reported</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$suspect</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>$reason</td>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>
							<a href='view-reported.php?edit=$id'><i class='icon-pencil2'></i></a>
							</td>
						</tr>
					";
				}
				...
				if ($total > 0) {
					echo "<br /><br /><center>Other Reported Users: ";
					for($n = 1; $n <= $paginas; $n++) {
				...
				$busca_it = mysql_query("SELECT * FROM meh_users_reports WHERE ((`suspect` LIKE '%".$busca_prot."%') OR ('%".$busca_prot."%'))");
				$conta_it = mysql_num_rows($busca_it);
				if($conta_it > 0){
					while($fetch_it = mysql_fetch_array($busca_it)){
						$id = $fetch_it['id'];
						$reported = $fetch_it['reported_by'];
						$suspect = $fetch_it['suspect'];
						$reason = $fetch_it['reason'];
					...
					echo "
						<tr>
							<td style='border: 1px #000; padding: 10px 50px 10px 50px;' align='center'>POST</td>
